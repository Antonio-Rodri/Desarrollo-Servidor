...
foreach ($juegos as $juego) {
    ...
    if (isset($_SESSION['user']) && $_SESSION['user']->tipo == "admin") {
        if ($juego->alquilado == "SI"){
            $j = alquilerController::buscarAlquilador($juego->codigo);
            echo "<br><p style='color: blue; font-size: 18px'>Juego alquilado por $j[Nombre] $j[Apellidos]</p>";
        }
        echo "<form action='' method='POST'><br><button type='submit' name='modificar' value='$juego->codigo'>Modificar</button>";
        echo "<button type='submit' name='borrar' value='$juego->codigo'>Borrar</button></form>";
    }
}
?>