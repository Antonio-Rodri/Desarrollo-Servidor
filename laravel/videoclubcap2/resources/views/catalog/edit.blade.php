    ...
    <x-layouts::auth>
        <div class="flex flex-col gap-6">
            <form method="POST" action="{{ route('catalog') }}" class="flex flex-col gap-6">
                @csrf
                @method('PUT')
                <flux:input name="titulo" :label="__('Título')" :value="old('titulo', $Pelicula['title'])"
                    type="text" required autofocus autocomplete="titulo" :placeholder="__('Título de la película')" />
